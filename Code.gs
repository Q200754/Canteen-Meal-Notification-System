...
<body class="p-3 p-md-4">
  ...
  <script>
    ...
    function goToAdmin() {
      // ใส่ URL หน้า Admin ของ Google Apps Script โดยตรง
      window.location.href = "https://script.google.com/macros/s/AKfycbzeqdLsPcZMePUnknJ19NueWcP2ZJV9WnuCJuSaSI1pgLU62fgbNIGiw8a5yFXo-GiTPQ/exec?page=admin";
    }
    ...
    function renderEvents(events) {
      ...
      container.innerHTML = events.map(e => {
        ...
        return \`
          <div class="col-12">
            <div class="card card-event p-3 p-md-4">
              <div class="d-flex justify-content-between align-items-center mb-2">
                <span class="badge \${badgeColor} px-3 py-2 rounded-pill">\${e.guestStatus}</span>
                <small class="text-muted fw-semibold">📅 \${e.date} | ⏰ \${e.time}</small>
              </div>
              <h5 class="fw-bold text-dark mb-2">\${e.occasion}</h5>
              <p class="text-muted mb-2" style="font-size: 0.95rem;">
                <strong>เจ้าภาพ/แขก:</strong> \${e.guestName} (\${e.guestCount} ท่าน) 
                \${e.hasFamily === 'ใช่' ? '<span class="badge bg-info-subtle text-info-emphasis ms-1">👨‍👩‍👧‍👦 มีครอบครัว/ผู้ติดตาม</span>' : ''}
              </p>
              <hr class="my-2">
              <div class="row g-2 mt-1">
                <div class="col-md-6">
                  <small class="text-muted d-block">🍱 รายการอาหารหลัก:</small>
                  <strong class="text-success">\${e.mainMenu}</strong>
                </div>
                <div class="col-md-6">
                  <small class="text-muted d-block">🍦 ของหวาน / ของทานเล่น:</small>
                  <strong class="text-warning-emphasis">\${e.snackDessert || '-'}</strong>
                </div>
              </div>
              \${e.note ? \`<div class="mt-2 text-secondary small">📌 <strong>หมายเหตุ:</strong> \${e.note}</div>\` : ''}
            </div>
          </div>
        \`;
      }).join('');
        ...
        <form id="adminForm" onsubmit="handleFormSubmit(event)">
          ...
          <div class="mb-3 p-3 bg-light rounded-3">
            <div class="form-check">
              ...
              <label class="form-check-label fw-medium" for="hasFamily">
                👨‍👩‍👧‍👦 มีครอบครัว / ผู้ติดตามมาด้วย
              </label>
            </div>
          </div>
          ...
          <div class="d-flex gap-2">
            <button type="submit" id="btnSubmit" class="btn btn-primary btn-submit flex-grow-1">
              💾 บันทึกข้อมูล & ส่ง LINE Alert
            </button>
            <button type="button" id="btnReset" class="btn btn-outline-secondary px-4 rounded-3" onclick="resetAdminForm()">
              ล้างฟอร์ม
            </button>
          </div>
        </form>
  ...
  <script>
    ...
    function renderAdminTable(events) {
      ...
      tbody.innerHTML = events.map(e => \`
        <tr>
          <td><small class="fw-bold">\${e.date}</small><br><small class="text-muted">\${e.time}</small></td>
          <td><strong>\${e.occasion}</strong></td>
          <td><small>\${e.guestName} (\${e.guestCount} คน)</small></td>
          <td>
            <span class="badge \${e.guestStatus === 'มา' ? 'bg-success' : e.guestStatus === 'รอยืนยัน' ? 'bg-warning text-dark' : 'bg-danger'}">
              \${e.guestStatus}
            </span>
          </td>
          <td style="text-align: center;">
            <button class="btn btn-sm btn-outline-primary me-1" onclick="editEvent('\${e.id}')" title="แก้ไข">✏️</button>
            <button class="btn btn-sm btn-outline-danger" onclick="deleteEvent('\${e.id}')" title="ลบ">🗑️</button>
          </td>
        </tr>
      \`).join('');
    }
